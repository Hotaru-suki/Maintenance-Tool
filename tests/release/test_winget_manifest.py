...
def test_generate_manifest_text_uses_expected_fields() -> None:
    text = generate_manifest_text(
        version="0.1.0",
        installer_url="https://example.invalid/MaintenanceTool-v0.1.0-win-x64-setup.exe",
        installer_sha256="abc123",
    )

    assert 'PackageIdentifier: "HotaruSuki.MaintenanceTool"' in text
    assert 'PackageVersion: "0.1.0"' in text
    assert 'InstallerType: "inno"' in text
    assert 'InstallerUrl: "https://example.invalid/MaintenanceTool-v0.1.0-win-x64-setup.exe"' in text
...
def test_write_manifest_creates_output_file(tmp_path: Path) -> None:
    output_path = tmp_path / "MaintenanceTool-v0.1.0-win-x64-winget.yaml"
